...
app.config['SECRET_KEY'] = os.environ.get(
    'SECRET_KEY', 'bruja_teatral_secret_key_2024_CHANGE_IN_PRODUCTION')
...
def init_db():
    ...
    try:
        ...
        if not cursor.fetchone():
            ...
            cursor.execute(
                'INSERT INTO users (username, password_hash) VALUES (?, ?)', ('admin', hashed_pw))
        ...
        if len(parts) != 3:
            if len(parts) == 2:
                ...
                expected_signature = hashlib.sha256(
                    (username + app.config['SECRET_KEY']).encode()).hexdigest()
    ...
    try:
        ...
        if user and check_password_hash(user['password_hash'], password):
            ...
            expires_at = int(time.time()) + \
                (app.config['TOKEN_EXPIRATION_HOURS'] * 3600)
...
def health():
    try:
        ...
        upload_writable = os.access(
            upload_folder, os.W_OK) if upload_exists else False

        # Count images
        image_count = 0
        if upload_exists:
            try:
                image_count = len([f for f in os.listdir(upload_folder)
...
def upload_file():
    ...
    try:
        ...
        if file_ext not in allowed_extensions:
            return jsonify({'error': 'Formato de archivo no permitido. Solo imágenes.'}), 400
        ...
        # Retornar URL relativa consistente con lo que espera el frontend
...
def get_images():
    ...
    try:
        upload_folder = app.config['UPLOAD_FOLDER']
        logger.info(f"Getting images from folder: {upload_folder}")
        ...
        for filename in os.listdir(upload_folder):
            ...
            # Solo agregar si es archivo y tiene extensión de imagen
            if os.path.isfile(full_path) and filename.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp')):
                images.append({
                    'filename': filename,
